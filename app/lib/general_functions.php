...
function updateprofile($user_id, $cnic_number, $passport_number, $sess_key)
{
   

    if (!empty($user_id) && !empty($cnic_number) && !empty($passport_number) && !empty($sess_key)) {
if( strlen($cnic_number) < 10 ){
    jsonencode(Null, 'CNIC number must be greater then 10 digits');
                exit();
}
if(strlen($cnic_number) > 15){
    jsonencode(Null, 'CNIC number must be less then 15 digits');
                exit();
}


        $data['cnic'] = $cnic_number;
        $data['passport'] = $passport_number; 
        $conn = Opencon();
        $rows = mysqli_query($conn, "SELECT sess_key from user_sessions where sess_key='$sess_key' and `user_id`='$user_id' and isactive='1' ");
        if (mysqli_num_rows($rows) > 0) {   

            $query=mysqli_query($conn,"SELECT user_id FROM kyc where user_id='$user_id'");

            if(mysqli_num_rows($query) ==1){
                
                $query=mysqli_query($conn,"UPDATE `kyc` SET `cnic_number`='$cnic_number',`passport_number`='$passport_number',`isActive`='1',`updated_at`=current_timestamp() where user_id='$user_id'");
                jsonencode($data,"User profile updated");
                exit();
            }
            elseif(mysqli_num_rows($query) >1 ){
                jsonencode(Null, 'You have more then one account');
                exit();
            }
            else{
                $query=mysqli_query($conn,"INSERT INTO `kyc`( `user_id`, `cnic_number`, `passport_number`, `isActive`, `create_at`) VALUES ('$user_id','$cnic_number','$passport_number','1',current_timestamp())");
                jsonencode($data, 'User profile inserted');
                exit();
            }
            ...
            jsonencode(Null, 'Error on UpdateProfile');
...
function jsonencode($data, $message = 'Not Found')
{
    ...
    if (!empty($data)) {
        $myJSON = json_encode(['status' => true , 'data' => $data, 'message' => $message]);
    } else {
